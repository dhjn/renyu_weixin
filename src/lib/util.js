...
export const encryptGzh = (level, params) => {
  delete params._sig
  params._aid = pubsource.applicationGzhId
  params._comid = pubsource.companyId
  //增加对data的trim处理  2018-10-17 darren
  for (let n in params) {
    // console.info(typeof params[n])
...
export const encryptNewGzh = (level, params)=> {
...
export const rlqtObject = (str) => {
  ...
  let name = str.substr(pos).replace('[rlqt]','').replace('[/rlqt]','')
...
//  过滤图片格式
export const filterPicture = (item) => {
  if (item.includes('gif') || item.includes('png') || item.includes('PNG') || item.includes('JPG') || item.includes('JPEG') || item.includes('jpg') || item.includes('jpeg')) {
    return true
  } else {
    return false
  }
}
// 转换时间格式 2019-09-12 -> 2019/09/12
export const rateTime = (time) => {
  let t1 = time.split(' ')
  let str = t1[0]
...
export const encryptNew = (level, params)=> {
  ...
  if (params.resId != undefined){
    param.resId = params.resId
    delete params.resId;
  }
  if (params.resType != undefined){
    param.resType = params.resType
...
export const encryptNew2 = (level, params)=> {
  ...
  if (params.empgid != undefined){
    param.empgid = params.empgid
    delete params.empgid;
  }
